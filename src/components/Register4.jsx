...
import SplashScreen  from '../Resgistration/SplashScreen';
import { FormInput } from '../Resgistration/FormComponents';
import {  FormSelect} from '../Resgistration/FormComponents';
...
export default function Register4() {
  ...
  const onSubmit = async (data) => {
    setIsLoading(true);
    try {
      if (!captchaToken) { recaptchaRef.current.execute(); return; }
      await axios.post(API_URL, { ...data, captchaToken: captchaToken }, { headers: { 'Content-Type': 'application/json' } });
    ...
    <div className="relative min-h-screen w-full flex flex-col font-sans bg-[#000000] text-white selection:bg-blue-500/30 overflow-x-hidden">
      ...
      {!showSplash && (
        <div className="relative z-10 flex flex-col w-full items-center justify-center p-4 lg:p-10 pb-10">
          ...
          <div className="hidden"><ReCAPTCHA ref={recaptchaRef} sitekey={RECAPTCHA_SITE_KEY} onChange={(token) => setCaptchaToken(token)} theme="dark" size="invisible" /></div>
        </div>
      )}
    </div>
  );
